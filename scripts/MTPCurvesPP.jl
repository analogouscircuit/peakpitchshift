...
τ200  = 0.025
τ2500 = 0.025
maxpeaks = Int(floor(Envelopes.hcc_maxdelay(1/f0; τ200=τ200, τ2500=τ2500) * f0))
# maxpeaks = 4
println("Number of peaks: ", maxpeaks)
...
@time for mth ∈ mtps
	...
	τ0s = []
	for sac ∈ sacs_p[mth]
		pks = PeakPick.hcpeaks(τ, 1/f0, sac; peakf=peakf)
		τ0 = EdgePitchUtils.delayfrompeaks(pks; maxpeaks=maxpeaks)
		push!(peaks, pks)
		push!(τ0s, τ0)
	end
	push!(peaks_p, peaks)
...
p_peaks = plot(τ, sacs_p[3][13])
DahlPlot.plotpeaks(p_peaks, peaks_p[3][13], 1.0; color=:green)
